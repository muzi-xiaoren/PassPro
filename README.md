...
| 查询 | 新增 | 列表 | 设置 |
| :--: | :--: | :--: | :--: |
| <img src="screenshots/android-query.jpg" width="200" alt="查询" /> | <img src="screenshots/android-add.jpg" width="200" alt="新增" /> | <img src="screenshots/android-list.jpg" width="200" alt="列表" /> | <img src="screenshots/android-settings.jpg" width="200" alt="设置" /> |
...
1. 新建一个 **私有仓库**（例：`pass`）。**无需勾选「初始化仓库」**——首次推送时 App 会自动创建 `passwords.log` 并建立 `master` 分支。
2. 创建私人令牌，权限选择 `projects`。
3. App 设置页填写：
  - Role：Mirror（也可作为 Primary）
  - Owner：仓库地址里的**命名空间**——即 `gitee.com/<这里>/<仓库名>`。例如仓库是 `gitee.com/ren_yeren/pass`，Owner 就填 `ren_yeren`（不是显示昵称）。
  - Repo：仓库名
  - Branch：`master`
  - File path：`passwords.log`
  - Token：对应账号的 Gitee 私人令牌（须有该仓库写权限）

> Owner / Repo 填错时，「测试连接」会报"仓库不存在或令牌无权访问"（Gitee 返回 HTTP 404 `Not Found Project`）。GitHub 同理：Owner = `github.com/<这里>/<仓库名>` 的命名空间。

### WebDAV / 坚果云

1. 注册坚果云账号。
2. 在坚果云「账户设置 → 安全选项 → 第三方应用管理」创建应用密码。
3. **先在坚果云里手动建好同步文件夹**（例：`PassPro`）。坚果云**不能**通过 WebDAV 自动创建顶层文件夹，必须先建好，且下面的「远程文件路径」要与之对应。
4. App 设置页填写：
  - Role：Primary 或 Mirror
  - 账户信息：坚果云账号**邮箱**
  - 服务器地址：`https://dav.jianguoyun.com/dav/`
  - 远程文件路径：`/PassPro/passwords.log`（其中 `/PassPro/` 必须是第 3 步建好的文件夹）
  - 应用密码：坚果云生成的**应用密码**（不是登录密码）

> 若文件夹不存在，「测试连接」会提示"目标文件夹不存在"（坚果云返回 HTTP 409）。按第 3 步在坚果云建好对应文件夹即可。文件 `passwords.log` 本身不用手动建，首次推送时会自动创建。
